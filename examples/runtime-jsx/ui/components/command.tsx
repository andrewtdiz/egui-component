...
export function CommandPanel({ id, query, placeholder, items, onCommand, inputProps, className }: { id: string; query: string; placeholder: string; items: Item[]; onCommand?: Handler; inputProps: Record<string, unknown>; className?: string }) {
  return (
...
export function CommandList({ id = "command-list", items = [], onCommand, className, children }: NodeProps & { items?: Item[]; onCommand?: Handler } & Record<string, unknown>) {
  return (
...
export function CommandItem({ item, index = 0, onCommand, className, ...props }: NodeProps & { item: Item; index?: number; onCommand?: Handler } & Record<string, unknown>) {
  ...
  const metadata = { item_id: id, item_label: label, item_index: index, item };
  ...
  return (
    <Button
      ...
      onClick={(event) => onCommand?.({ ...(event as object), metadata }, id)}
    />
  );
}

...
export function Checkbox({ children, checked, value, label, className, ...props }: NodeProps & { checked?: boolean; value?: boolean; label?: string } & Record<string, unknown>) {
  ...
  return <input data-slot="checkbox" type="checkbox" {...nodeProps({ ...props, id: requireNodeId(props, "Checkbox") }, className)} checked={resolvedValue} label={label ?? textFromChildren(children)} />;
}

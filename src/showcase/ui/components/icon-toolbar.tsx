...
function itemEvent(event: unknown, item: Item, index: number, id: string, label: string) {
  ...
  return { ...base, value: id, metadata: { ...metadata, item_id: id, item_label: label, item_index: index, item } };
}
...
export function IconToolbarItemButton({ item, index = 0, selected = false, baseId = "icon-toolbar", onSelect, className, ...props }: NodeProps & { item: Item; index?: number; selected?: boolean; baseId?: string; onSelect?: Handler } & Record<string, unknown>) {

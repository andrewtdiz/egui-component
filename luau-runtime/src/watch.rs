...
mod tests {
    ...
    fn modify_create_remove_and_rename_mark_project_modules_dirty() {
        let root = normalize_path(&runtime_example_path("demo.luau")).unwrap();
    ...
    fn nested_root_uses_luaurc_project_root_for_watch_scope() {
        let root = normalize_path(&runtime_example_path("showcase/main.luau")).unwrap();
        let watched_dir = resolve_watched_dir(&root).unwrap();
    ...
    fn nested_root_detects_sibling_modules_under_project_root() {
        ...
        let watched_dir = resolve_watched_dir(&root).unwrap();
        let helper =
            normalize_path(&runtime_example_path("components/profile_panel.luau")).unwrap();
        let event = Event {
            kind: EventKind::Modify(ModifyKind::Any),

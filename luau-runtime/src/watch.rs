...
/// Watches a Luau project rooted at one root script path and enqueues dirty module paths.
...
impl RootScriptWatcher {
    ...
    pub fn new(
        ...
        let watched_dir = watched_root.parent().ok_or_else(|| {
            RuntimeError::config(format!(
                "root script path `{}` has no parent directory",
                watched_root.display()
            ))
        })?;
        let watched_dir = watched_dir.to_path_buf();
...
mod tests {
    ...
    fn modify_create_remove_and_rename_mark_project_modules_dirty() {
        let root = normalize_path(Path::new("examples/runtime-luau/demo.luau")).unwrap();
        let watched_dir = root.parent().unwrap().to_path_buf();
        let leaf = normalize_path(Path::new("examples/runtime-luau/leaf.luau")).unwrap();
    ...
    fn non_luau_or_out_of_tree_paths_are_ignored() {
        let root = normalize_path(Path::new("examples/runtime-luau/demo.luau")).unwrap();
        let watched_dir = root.parent().unwrap().to_path_buf();
        let png = normalize_path(Path::new("examples/runtime-luau/preview.png")).unwrap();
        let elsewhere = normalize_path(Path::new("Cargo.toml")).unwrap();
        let event = Event {
